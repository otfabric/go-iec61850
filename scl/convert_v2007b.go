...
import (
	...
	raw "github.com/otfabric/go-iec61850/scl/internal/raw/v2007b"
)

func convertV2007B(r *raw.SCL) (*SCL, []Diagnostic, error) {
	...
	for _, sub := range r.Substation {
		s.Substations = append(s.Substations, convSubstationB(sub))
	}
	if r.Communication != nil {
		s.Communication = convCommB(r.Communication)
	}
	for _, ied := range r.IED {
		s.IEDs = append(s.IEDs, convIEDB(ied, &diags))
	}
	if r.DataTypeTemplates != nil {
		s.DataTypeTemplates = convDTTB(r.DataTypeTemplates, &diags)
	}

	return s, diags, nil
}

func convIEDB(r raw.IED, diags *[]Diagnostic) IED {
	ied := IED{
		Name:         r.Name,
		Desc:         r.Desc,
	...
	if r.Services != nil {
		ied.Services = convServicesB(r.Services)
	}
	for _, ap := range r.AccessPoint {
		ied.AccessPoints = append(ied.AccessPoints, convAPB(ap, diags))
	}
...
func convAPB(r raw.AccessPoint, diags *[]Diagnostic) AccessPoint {
	ap := AccessPoint{Name: r.Name}
	if r.Server != nil {
		srv := &Server{}
		for _, ld := range r.Server.LDevice {
			srv.LDevices = append(srv.LDevices, convLDB(ld, diags))
		}
		ap.Server = srv
	}
	for _, p := range r.Private {
...
func convLDB(r raw.LDevice, diags *[]Diagnostic) LDevice {
	ld := LDevice{Inst: r.Inst, Desc: r.Desc}
	ln0 := convLN0B(r.LN0, diags)
	ld.LN0 = &ln0
	for _, ln := range r.LN {
		ld.LNs = append(ld.LNs, convLNB(ln, diags))
	}
	for _, p := range r.Private {
...
func convLN0B(r raw.LN0, _ *[]Diagnostic) LN {
	ln := LN{LNClass: r.LnClass, Inst: r.Inst, LNType: r.LnType, Desc: r.Desc}
	for _, doi := range r.DOI {
		ln.DOIs = append(ln.DOIs, convDOIB(doi))
	}
	for _, ds := range r.DataSet {
		ln.DataSets = append(ln.DataSets, convDSB(ds))
	}
	for _, rc := range r.ReportControl {
		ln.Reports = append(ln.Reports, convRCB(rc))
	}
...
func convLNB(r raw.LN, _ *[]Diagnostic) LN {
...
func convDOIB(r raw.DOI) DOI {
	doi := DOI{Name: r.Name, Desc: r.Desc}
	for _, d := range r.DAI {
		doi.DAIs = append(doi.DAIs, convDAIB(d))
	}
	for _, s := range r.SDI {
		doi.SDIs = append(doi.SDIs, convSDIB(s))
	}
	return doi
}

func convSDIB(r raw.SDI) SDI {
	sdi := SDI{Name: r.Name}
	for _, d := range r.DAI {
		sdi.DAIs = append(sdi.DAIs, convDAIB(d))
	}
	for _, s := range r.SDI {
		sdi.SDIs = append(sdi.SDIs, convSDIB(*s))
	}
	return sdi
}

func convDAIB(r raw.DAI) DAI {
	val := ""
	if len(r.Val) > 0 {
		val = r.Val[0].Value
	}
...
func convDSB(r raw.DataSet) DataSet {
...
func convRCB(r raw.ReportControl) ReportControl {
	rc := ReportControl{
...
func convDTTB(r *raw.DataTypeTemplates, diags *[]Diagnostic) DataTypeTemplates {
...
func convSubstationB(r raw.Substation) Substation {
	sub := Substation{Name: r.Name, Desc: r.Desc}
	for _, ln := range r.LNode {
		sub.LNodes = append(sub.LNodes, convLNodeB(ln))
	}
...
func convLNodeB(r raw.LNode) LNode {
	return LNode{
...
func convCommB(r *raw.Communication) *Communication {
	comm := &Communication{}
...
func convServicesB(r *raw.Services) *Services {
	svc := &Services{
